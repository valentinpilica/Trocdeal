...
class Comments extends CI_Controller
{
	...
	private function _init()
	{
		...
		$this->output->set_title('Comments | TrocDeal - Business changing between products');
	 ...
	 * After a user logs in, index() is called and the main
	...
	function index()
	{
		...
		$this->breadcrumbs->push('Comments', '/comments/index');
		$this->breadcrumbs->unshift('Home', '/');
		
		$this->load->section('breadcrumbs', 'layout/' . $this->config->item('default_theme_name') . '/sections/breadcrumbs/breadcrumbs');
		$this->load->view('layout/' . $this->config->item('default_theme_name') . '/sections/comments/comments', $data);
	}
