...
class Profile extends CI_Controller
{
	...
	private function _init()
	{
		...
		$this->output->set_title('Profile | Trocdeal - Business changing between products');	
	...
	{
		$this->breadcrumbs->push('Profile', '/profile/index');
		$this->breadcrumbs->unshift('Home', '/');
		$this->load->section('breadcrumbs', 'layout/' . $this->config->item('default_theme_name') . '/sections/breadcrumbs/breadcrumbs');
		$this->load->view('layout/' . $this->config->item('default_theme_name') . '/sections/profile/profile');
	}
}
