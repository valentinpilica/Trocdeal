...
class MY_Loader extends CI_Loader
{
	...
	function css()
	{
		...
		foreach ($css_files as $css_file)
		{
			...
			if (! $is_external) if (! file_exists($css_file)) show_error("Cannot locate stylesheet file: {$css_file}.");
			
			$css_file = $is_external == FALSE ? base_url() . $css_file : $css_file;
			
			if (! in_array($css_file, $this->_css)) $this->_css[] = $css_file;
	...
	function js()
	{
		...
		foreach ($script_files as $script_file)
		{
			...
			if (! $is_external) if (! file_exists($script_file)) show_error("Cannot locate javascript file: {$script_file}.");
	...
	function audio()
	{
		...
		foreach ($script_files as $script_file)
		{
			...
			if (! $is_external) if (! file_exists($script_file)) show_error("Cannot locate audio file: {$script_file}.");
				
			$script_file = $is_external == FALSE ? base_url() . $script_file : $script_file;
				
			if (! in_array($script_file, $this->_audio)) $this->_audio[] = $script_file;
		}
	...
	function get_cached_audio_files()
	{
		return $this->_cached_audio;
	}	
	...
	{
		$section_string = '';
		if (isset($this->_sections[$section_name])) foreach ($this->_sections[$section_name] as $section)
	...
	/*
	 * Can load a view file from an absolute path and relative to the CodeIgniter index.php file Handy if you have views outside the usual CI views dir
	 */
